...
put(Cache, Key, Value) ->
    gen_server:call(Cache, {put, Key, Value}, infinity).
...
%%--------------------------------------------------------------------
init([]) ->
    cherly:start(list_to_integer(couch_config:get("couchdb", "btree_cache_bytes", "33554432"))).
...
handle_call({put, Key, Val}, _From, Cache) ->
    {reply, {ok, cherly:put(Cache, Key, Val)}, Cache}.
